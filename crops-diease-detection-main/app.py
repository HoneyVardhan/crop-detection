...
# ✅ Model path (adjust filename if different)
...
def predict():
    ...
    try:
        ...
        if predictions.ndim != 2 or predictions.shape[1] != len(class_names):
            raise ValueError(f"Model output mismatch: expected {len(class_names)} classes but got {predictions.shape[1]}")
...
# ✅ Run app (works for both local & Vercel)
